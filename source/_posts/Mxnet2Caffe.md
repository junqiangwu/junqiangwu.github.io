...
转换的时候如果存在caffe不支持的op，**需要自己添加自定义层**，否则在构建网络时，会error，本工程添加了`broadcast_mul`层caffe添加自定义层的介绍比较多，就跳过了
...
然后利用Caffe 加载刚才转换的网络，打印输出，对比结果精度，如果出现问题，则需要逐层排查，本工程在`SENet`网络上测试正常



[工程项目地址](https://github.com/junqiangwu/Mxnet2Caffe-Tensor-RT-SEnet)
